...
def encoding_seq_np( seq, arr):
  for i, c in enumerate(seq):
    if c == "_":
      ...
      arr[ idx ] = 1
    elif isinstance(CHARSET[ c ], tuple) and len(CHARSET[ c ]) == 2: 
...
class DataSet(object):
  def __init__(self, fpath, seqlen, n_classes=86, need_shuffle=True, size_data=None):
    self.SEQLEN = seqlen
  ...
  def read_raw(self, fpath):
    ...
    with open( fpath, 'r') as tr:
      for row in tr.readlines():
        (label1, label2, label3, seq) = row.strip().split()
        seqlen = len(seq)
        ...
        res.append( (label1, label2, label3, seq) )
    return res
  ...
  def parse_data(self, idxs, with_raw=False):
    ...
    labels1 = np.zeros( (isize, self.NCLASSES), dtype=np.uint8 )    
    labels2 = np.zeros( (isize, self.NCLASSES), dtype=np.uint8 )    
    labels3 = np.zeros( (isize, self.NCLASSES), dtype=np.uint8 )    
    raw = []

    for i, idx in enumerate(idxs):
      label1, label2, label3, seq = self._raw[ idx ]

      encoding_label_np(label1, labels1[i] )
      encoding_label_np(label2, labels2[i] )
      encoding_label_np(label3, labels3[i] )
      encoding_seq_np(seq, data[i] )

    if with_raw:
      return ( data.reshape(isize, self.SEQLEN, CHARLEN), labels1, labels2, labels3, raw )
    else:
      return [ data.reshape(isize, self.SEQLEN, CHARLEN), labels1, labels2, labels3 ]
